...
function LotsStock() {
  ...
  const loadDonneesStock = useCallback(async () => {
    ...
    try {
      ...
      const resProd = await axios.get("https://postgres-production-2352.up.railway.app/api/produit", getAxiosConfig());
      setProduits(resProd.data);

      // 2. Charger les alertes de péremption (lots à -90 jours ou périmés)
      const resAlertes = await axios.get("https://postgres-production-2352.up.railway.app/api/lots/alertes-peremption", getAxiosConfig());
  ...
  const handleEntreeStock = async (e) => {
    ...
    try {
      await axios.post("https://postgres-production-2352.up.railway.app/api/lots", payload, getAxiosConfig());
  ...
  const handleRetraitPerime = async (id_lot) => {
    ...
    try {
      await axios.post(
        `https://postgres-production-2352.up.railway.app/api/lots/retrait-perime/${id_lot}`,
        {
