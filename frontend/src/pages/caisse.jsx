...
// Configuration de la base URL de l'API (adaptable selon l'environnement Vite/CRA)
const BASE_URL =
  process.env.REACT_APP_API_URL ||
  (import.meta && import.meta.env && import.meta.env.VITE_API_URL) ||
  "https://pharmacie-production-9a16.up.railway.app";
...
function Caisse() {
  ...
  // 1. Récupération dynamique de l'ID de Structure
  const getStructureId = useCallback(() => {
    const localId =
      localStorage.getItem("id_structure") ||
      localStorage.getItem("structureId");
    ...
    if (storedUser) {
      try {
        ...
        return (
          parsed?.id_structure ||
          parsed?.structure?.id_structure ||
          parsed?.structure_info?.id_structure ||
          parsed?.structureId
        );
      } catch (e) {
        console.error("Erreur de lecture de id_structure:", e);
      }
    }
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        console.error("Erreur de lecture du user local", e);
      }
    }
    ...
    if (storedStructure) {
      try {
        ...
        console.error("Erreur parse structure locale", e);
      }
    }
  }, []);

  // 3. Appels API centralisés
  const loadStructureConfig = useCallback(async () => {
    const idLocalStorage = getStructureId();
    if (!idLocalStorage) return;

    try {
      const r = await axios.get(`${BASE_URL}/api/structure`, getAxiosConfig());

      if (r.data && Array.isArray(r.data)) {
        const structureTrouvee = r.data.find(
          (str) =>
            str.id_structure === idLocalStorage || str.id === idLocalStorage
        );
  ...
  const loadProduits = useCallback(async () => {
    ...
    if (!idStructure) return;
    try {
      const r = await axios.get(`${BASE_URL}/api/produit`, getAxiosConfig());
      setProduits(r.data);
    } catch (error) {
  ...
  const loadVentesRecentes = useCallback(async () => {
    ...
    if (!idStructure) return;
    try {
      const r = await axios.get(`${BASE_URL}/api/vente`, getAxiosConfig());
  ...
  const totalBrut = useMemo(() => {
    return panier.reduce(
      (sum, item) =>
        sum +
        parseFloat(item.prix_vente_unitaire || 0) *
          (parseInt(item.quantite_panier, 10) || 0),
      0
    );
  }, [panier]);
  ...
  const handleModePaiementChange = (e) => {
    ...
    const abonneTrouve = abonnements.find(
      (sub) => sub.nom === valeurSelectionnee
    );
    if (abonneTrouve) {
  ...
  const handleOuvrirFacture = async (vente) => {
    try {
      const res = await axios.get(
        `${BASE_URL}/api/vente/details/${vente.id_vente}`,
        getAxiosConfig()
      );
  ...
  const validerVente = async (e) => {
    ...
    if (!idStructure) {
      return alert(
        "Erreur : L'identifiant de la structure est introuvable. Veuillez vous reconnecter."
      );
    }
    if (panier.length === 0) return alert("Le panier est vide.");

    const aDesErreurs = panier.some(
      (item) => !item.quantite_panier || item.quantite_panier <= 0
    );
    if (aDesErreurs) {
        ...
        <div className="row">
          ...
          <div className="col-md-8 mb-4">
            <div className="card p-3 shadow-sm mb-3 position-relative">
              ...
              {produitsFilitres.length > 0 && (
                <ul
                  className="list-group position-absolute left-0 w-100 shadow-lg"
                  style={{ zIndex: 1000, top: "75px" }}
                >
                  {produitsFilitres.map((p) => (
                    ...
                    >
                      ...
                      <span>
                        {parseFloat(p.prix_vente_unitaire).toLocaleString()}{" "}
                        FCFA | Stock:{" "}
                        <strong
                          className={
                            parseInt(p.stock_total, 10) <= 5
                              ? "text-danger"
                              : "text-success"
                          }
                        >
                    ...
                    {panier.map((item) => (
                      <tr key={item.id_produit}>
                        ...
                        <td className="fw-bold">
                          {(
                            parseFloat(item.prix_vente_unitaire) *
                            (parseInt(item.quantite_panier, 10) || 0)
                          ).toLocaleString()}{" "}
                          FCFA
                        </td>
          ...
          <div className="col-md-4 mb-4">
            <div className="card p-3 shadow-sm bg-dark text-white h-100 d-flex flex-column justify-content-between">
              <div>
                <h5 className="fw-bold text-uppercase text-muted border-bottom pb-2">
                  Résumé Vente
                </h5>
                ...
                <div className="mb-3">
                  <label className="form-label small fw-bold text-muted">
                    Mode de Règlement / Abonnement
                  </label>
                  <select
                    className="form-select bg-secondary text-white border-0 mb-3"
                    value={modePaiement}
                ...
                <div className="mb-3 p-2 border border-secondary rounded bg-transparent">
                  ...
                  <small className="text-muted d-block mt-1">
                    Le caissier peut ajuster ce pourcentage librement.
                  </small>
                </div>
              </div>
                ...
                <table className="table table-sm table-hover border align-middle mb-0">
                  ...
                  <tbody>
                    {ventesRecentes.map((v) => {
                      const brutTotal = parseFloat(
                        v.total_sans_reduction || v.total_somme || 0
                      );
                      const netPaye = parseFloat(v.total_somme || 0);
                      const partAbonne = parseFloat(
                        v.total_prise_en_charge || 0
                      );
                      return (
                        <tr key={v.id_vente}>
                          <td>
                          ...
                          <td className="text-end text-muted">
                            {brutTotal.toLocaleString()} F
                          </td>
                          <td className="text-end text-danger fw-bold">
                            {partAbonne > 0
                              ? `-${partAbonne.toLocaleString()} F`
                              : "0 F"}
                          </td>
                          <td className="text-end fw-bold text-success">
                            {netPaye.toLocaleString()} FCFA
                          </td>
                          <td className="text-end">
                            <button
                              className="btn btn-sm btn-dark fw-bold"
                              onClick={() => handleOuvrirFacture(v)}
                            >
                              🖨️ Facture
                            </button>
                          </td>
                  ...
                  <div className="row small mb-3">
                    <div className="col-7">
                      ...
                      <span className="text-uppercase">
                        {venteSelectionnee.id_vente?.substring(0, 13)}
                      </span>
                      <br />
                      <strong>Date:</strong>{" "}
                      {new Date(venteSelectionnee.date_vente).toLocaleString()}
                      <br />
                    </div>
                    <div className="col-5 text-end">
                      <strong>Règlement:</strong>{" "}
                      {venteSelectionnee.mode_paiement}
                      <br />
                      <strong>Caissier:</strong>{" "}
                      {currentUser?.nom_utilisateur || "Opérateur"}
                    </div>
                  </div>
                    ...
                    <tbody>
                      {venteSelectionnee.items?.map((item, idx) => (
                        <tr key={idx}>
                          ...
                          <td className="text-end">
                            {parseFloat(
                              item.prix_unitaire_base ||
                                item.prix_unitaire_vendu
                            ).toLocaleString()}
                          </td>
                          <td className="text-end text-danger">
                            {item.taux_reduction > 0
                              ? `${item.taux_reduction}%`
                              : "0%"}
                          </td>
                          <td className="text-end fw-bold">
                            {(
                              item.quantite * item.prix_unitaire_vendu
                            ).toLocaleString()}
                          </td>
                        </tr>
                      ))}
                  ...
                  <div className="border-top pt-2 text-end small">
                    <div className="d-flex justify-content-between border-bottom pb-1 mb-1">
                      ...
                      <span className="fw-bold">
                        {parseFloat(
                          venteSelectionnee.total_sans_reduction || 0
                        ).toLocaleString()}{" "}
                        FCFA
                      </span>
                    </div>
                    {parseFloat(venteSelectionnee.total_prise_en_charge) >
                      0 && (
                      <div className="d-flex justify-content-between border-bottom pb-1 mb-1 text-danger fw-bold">
                        ...
                        <span>
                          -
                          {parseFloat(
                            venteSelectionnee.total_prise_en_charge
                          ).toLocaleString()}{" "}
                          FCFA
                        </span>
                      </div>
                    ...
                    <div className="d-flex justify-content-between pt-1">
                      ...
                      <h5 className="fw-bold text-success m-0">
                        {parseFloat(
                          venteSelectionnee.total_somme || 0
                        ).toLocaleString()}{" "}
                        FCFA
                      </h5>
                    </div>
