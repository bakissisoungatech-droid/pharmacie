...
function Inscription() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            const response = await axios.get(`postgres-production-2352.up.railway.app/api/utilisateur`, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editId) {
                await axios.put(`postgres-production-2352.up.railway.app/api/utilisateur/${editId}`, formData, {
                    headers: { "x-structure-id": structureId }
                });
                alert("Utilisateur modifié !");
            } else {
                await axios.post(`postgres-production-2352.up.railway.app/api/utilisateur/post`, formData, {
                    headers: { "x-structure-id": structureId }
                });
    ...
    const handleDelete = async (id_utilisateur) => {
        ...
        if (window.confirm("Voulez-vous vraiment supprimer cet utilisateur ?")) {
            try {
                await axios.delete(`postgres-production-2352.up.railway.app/api/utilisateur/${id_utilisateur}`, {
                    headers: { "x-structure-id": structureId }
                });
                loadData();
            } catch (error) {
                        ...
                        <div className="card p-4 shadow-sm border-0">
                            ...
                            <div className="table-responsive">
                                <ul className="list-group">
                                    {/* 🟢 CORRECTION ICI : Filtrage pour exclure les comptes "Proprio" ou "proprio" */}
                                    {data
                                        .filter((u) => u.role && u.role.toLowerCase() !== "proprio")
                                        .map((u) => (
                                    ...
                                    {/* Vérification ajustée sur la liste filtrée */}
                                    {data.filter((u) => u.role && u.role.toLowerCase() !== "proprio").length === 0 && (
